...
public class BuildItinerary {
	...
	public ArrayList<Flight> cheapestItinerary(String airportDepart,String airportDestin,
	   ...
	   // maxVertex=graph.getVertexAtPosition(9); maxTime=maxVertex.getElement().getTime();
	    ...
	    /********************** CODE TO BE ADDED HERE ******************************************/
	     // Part 1
		for (Vertex<TimeAtAirport> node: graph.vertices()  ){ // For-each loop that traverses every vertex in the graph
			
			Time time = ( node.getElement () ) .getTime(); // Will get the time of that specific vertex (node)
				
			if (node.getElement().getAirport().equals(airportDepart)){ // If the vertex airport matches the airportDepart demanded enter this statement
				
				if( ( time.compareTo(minTime) <= -1 ) && ( time.compareTo(minDepTime ) >= 1 ) ){ 
				...
				if ( node.getElement().getAirport().equals(airportDestin) ){// If the vertex airport matches the airporDestin demanded enter this statement
					
						
					if( ( time.compareTo(maxArrTime) <= -1) && (time.compareTo(maxTime) >= 1 ) ){
		...
		if (spInfo.isReachable(graph.getPositionOfVertex(maxVertex))) {
		   ...
		   Edge[] previousEdge = spInfo.getPrevEdge();
		  
		   int costAmount = 0;
		  
		   for (int i = 1; i < path.length; i++) {
			  
			   Flight flight = (Flight) previousEdge[path[i]].getElement();
		      	
			   if(flight.getAirportDepart().equals(flight.getAirportDestin()) == false){ //removing the possibly of airport being themselves       		 
				   costAmount = costAmount + flight.toIntegral();
				   System.out.println(previousEdge[path[i]].getElement());	      			
			   }
		    
		   }
		   
			System.out.println("Total Cost = $" + costAmount);	
		} // End of Part 1
		   
			else{ // will be entered if there are no itineraries available (Part 2)
	...
	public void whereMoneyCanGetMe(String airportDepart, Time minDepTime,int dollars, boolean verbose) {
		...
		//Part 3
		String printAirports = " ";
		int[] previousVertices = spInfo.getPrevVertex();
		
		for(Vertex<TimeAtAirport> vertex : graph.vertices()){
			
			int vertexPosition = graph.getPositionOfVertex(vertex);
			
			if(spInfo.isReachable(vertexPosition) && spInfo.getDist(vertexPosition) <= dollars){ //Each vertex in the graph will be getting verified to see if its reachable under the amount the person has
				
				 Vertex<TimeAtAirport> previousVertex = graph.getVertexAtPosition(previousVertices[vertexPosition]);
			
				 	if(vertex.getElement().getAirport().equals(previousVertex.getElement().getAirport()) == false){ //if the given vertex has the same airport as the previous vertex then it will be ignored
				 		
				 		printAirports = printAirports + "("+vertex.getElement().getAirport()+","+vertex.getElement().getTime()+"),";
				 }
			}
		}
		System.out.println("With $" +dollars+ ", you can go to: "+ printAirports);
